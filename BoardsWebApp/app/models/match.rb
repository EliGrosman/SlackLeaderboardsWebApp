class Match < ApplicationRecord
  ...
  before_create {
    ...
    if(winnerObj.nil?) 
      if(board.elo_enabled)
        con.execute("INSERT INTO '" + board.board_name + "'(player, wins, losses, elo) VALUES('" + winner + "', 1, 0," + newEloWinner.to_s + ")")        
      else
      ...
      wins = winnerObj[1] + 1
      if(board.elo_enabled)
        con.execute("UPDATE '" + board.board_name + "' SET wins = " + wins.to_s + ", elo = " + newEloWinner.to_s + " WHERE player = '" + winner + "'")
      else
    ...
    if(loserObj.nil?)
      if(board.elo_enabled)
        con.execute("INSERT INTO '" + board.board_name + "'(player, wins, losses, elo) VALUES('" + loser + "', 0, 1," + newEloLoser.to_s + ")")        
      else
      ...
      losses = loserObj[2] + 1
      if(board.elo_enabled)
        con.execute("UPDATE '" + board.board_name + "' SET losses = " + losses.to_s + ", elo = " + newEloLoser.to_s + " WHERE player = '" + loser + "'")
      else
